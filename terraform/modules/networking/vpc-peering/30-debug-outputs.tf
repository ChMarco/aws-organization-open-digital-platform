data "null_data_source" "debug_outputs" {
  ...
  inputs = {
    ...
    mgmt_subnets = "${data.aws_subnet_ids.mgmt_subnets.id}"
    mgmt_dmz_route_tables = "${data.aws_route_table.mgmt_dmz_route_tables.*.id}"
    mgmt_private_route_tables = "${data.aws_route_table.mgmt_private_route_tables.*.id}"
    mgmt_public_route_tables = "${data.aws_route_table.mgmt_public_route_tables.*.id}"
  }
}

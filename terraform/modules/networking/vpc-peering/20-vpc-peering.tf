...
resource "aws_route" "mgmt_to_standard_dmz" {
  ...
  route_table_id = "${element(split(",", join(",", data.aws_route_table.mgmt_dmz_route_tables.*.route_table_id)), count.index)}"
...
resource "aws_route" "mgmt_to_standard_public" {
  ...
  route_table_id = "${element(split(",", join(",", data.aws_route_table.mgmt_public_route_tables.*.route_table_id)), count.index)}"
  destination_cidr_block = "${var.vpc_cidr}"
  vpc_peering_connection_id = "${aws_vpc_peering_connection.vpc_peering_connection.id}"
}
...
resource "aws_route" "mgmt_to_standard_private" {
  ...
  route_table_id = "${element(split(",", join(",", data.aws_route_table.mgmt_private_route_tables.*.route_table_id)), count.index)}"
  destination_cidr_block = "${var.vpc_cidr}"
  vpc_peering_connection_id = "${aws_vpc_peering_connection.vpc_peering_connection.id}"
}

module "gocd" {
  ...
  gocd_public_key = "${var.management_keypair}"
  gocd_subnets = "${lookup(module.vpc.vpc_outputs, "public_subnet_ids")}"
  gocd_elb_subnets = "${lookup(module.vpc.vpc_outputs, "dmz_subnet_ids")}"
  gocd_ssh_bastion_access = "${lookup(module.bastion.bastion_outputs, "bastion_security_group_id")}"
  gocd_web_whitelist = "${var.ssh_web_whitelist}"
